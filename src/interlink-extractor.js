...
const getCleanScriptData = compose(discardEmpty, dataInScript);

const extractInterlinkHashes = compose(
  map(compose(head, drop(1))),
  filter(isInterlinkData),
  map(compose(getCleanScriptData, prop('script'))),
  filter(isBurnOutput),
  prop('outputs')
);

const extractInterlinkHashesFromMerkleBlock = compose(unnest, map(extractInterlinkHashes), prop('txs'));

module.exports = {extractInterlinkHashes, extractInterlinkHashesFromMerkleBlock};
